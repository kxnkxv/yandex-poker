import { IPlayer, Nullable, TEvaluatedHand, TPlayerPublic, TCardNames } from './types'
...
class Player implements IPlayer {
  ...
  sittingOnTable: boolean | number
  seat: Nullable<number>

  constructor(socket: any, name: string, chips: number) {
    this.public = {
   ...
   */
  sitOnTable(tableId: number, seat: number, chips: number) {
   ...
   */
  evaluateHand(board: string[]) {
    let cards = this.cards.concat(board)
    let cardNamess = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A']
    let cardNames: TCardNames = {
      A: 'ace',
      K: 'king',
      Q: 'queen',
      J: 'jack',
    ...
    let getCardName = function (cardValue: string, plural: boolean = false) {
      if (plural) {
    ...
    /*let swap = function (index1: number, index2: number) {
    ...
    let rateHand = function (hand: any) {
      return (
    ...
    let evaluatedHand: TEvaluatedHand = {
      rank: '',
      name: '',
      rating: 0,
      cards: [],
    ...
    for (let i = 1; i < openedCards.length; i++) {
      ...
      if (numberOfPairs) {
        ...
        // If there are two pairs
          ...
          if (
            ...
            (pairs[Object.keys(pairs)[0]].length == pairs[Object.keys(pairs)[1]].length &&
              cardNamess.indexOf(Object.keys(pairs)[0]) > cardNamess.indexOf(Object.keys(pairs)[1]))
          ) {
            evaluatedHand.cards = pairs[Object.keys(pairs)[0]]
